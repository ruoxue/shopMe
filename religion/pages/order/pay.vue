<template>
	<view class="container">
		...
		<scroll-view scroll-y class="margin-top-sm">
			<view class="cu-list menu ">
				<view class="cu-item arrow content" v-for="(item, index) in order.buyerItems" :index="index">
					...
					<image class="cu-avatar xl  margin-10 fl" style="width:180rpx;height: 180rpx; margin: 10rpx;"
						:src="item.thumb" :style="'background-image: url(' + item.thumb + ');'"></image>
		...
		<view style="width: 100vw; " class="margin-top-lg">
			<textarea class="bg-white  padding-sm" style="width: 94vw;margin-left: 3vw;"
				:placeholder="order.formateDesc" v-if="order.type==4" v-model="remark"> 备注信息</textarea>
			...
			getChannel() {
				...
				this.$net.fetch(
					function(ret) {
						...
						that.channel.id = that.channels[0].id;
					},
				...
				this.$net.fetch(
					function(ret) {
						if (ret.data.orderStatus == 10) {
							...
							if (ret.data.body != undefined) {
 
						
								const div = document.createElement('div') // 创建div
								div.innerHTML = ret.data.body // 将返回的form 放入div
