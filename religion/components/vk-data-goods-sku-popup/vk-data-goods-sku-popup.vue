				...
				<scroll-view class="specification-wrapper-content" scroll-y="true">
					<view class="specification-header">
						<view class="specification-left">
							...
							<image
							:src="selectShop.thumb?selectShop.thumb:goodsInfo.thumb"
								class="product-img"
...
export default {
		...
		return {
			...
			themeColor: {
				...
				default: {
					...
					buyNowColor: '#ffffff',
					buyNowBackgroundColor: 'rgb(254,0, 0)',
					addCartColor: '#ffffff',
					addCartBackgroundColor: 'rgb(255, 80, 2)',
					btnStyle: {
						color: '#333333',
						borderColor: '#f4f4f4',
	...
	methods: {
		...
		getSelectShop(arr){
			...
			var ret=this.permute(arr);
			
			for(var item in ret){
				var r=this.shopItemInfo[this.getArrayToSting(ret[item])];
				if(r)
				return r;
			}
		...
		checkSelectShop() {
			...
			if (that.selectArr.every(item => item != '')) {
				...
				that.selectShop = that.getSelectShop(that.selectArr);
			...
			for (let i = 0, len = specList.length; i < len; i++) {
				...
				for (let j = 0; j < len2; j++) {
					...
					let choosed_copy2 = choosed_copy.filter(item => item !== '' && typeof item !== 'undefined');
					  if (that.getSelectShop(choosed_copy2)) {
		...
		previewImage() {
			let that = this;
			let { selectShop, goodsInfo, text } = that;
			let src = selectShop.image ? selectShop.image : goodsInfo[text];
			if (src) {
				uni.previewImage({
					urls: [src]
				});
