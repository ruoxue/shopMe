    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['shop:gpptUser:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['shop:gpptUser:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="gpptUserList" @selection-change="handleSelectionChange">
...
<script>
import { listGpptUser, getGpptUser, delGpptUser, addGpptUser, updateGpptUser } from "@/api/shop/gpptUser";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listGpptUser(this.queryParams).then(response => {
        this.gpptUserList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getGpptUser(userId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.userId != null) {
            updateGpptUser(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGpptUser(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('shop/gpptUser/export', {
        ...this.queryParams
      }, `gpptUser_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
