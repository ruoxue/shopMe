    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['shop:gdptAdvert:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['shop:gdptAdvert:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['shop:gdptAdvert:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['shop:gdptAdvert:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="gdptAdvertList" @selection-change="handleSelectionChange">
...
<script>
import { listGdptAdvert, getGdptAdvert, delGdptAdvert, addGdptAdvert, updateGdptAdvert } from "@/api/shop/gdptAdvert";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listGdptAdvert(this.queryParams).then(response => {
        this.gdptAdvertList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getGdptAdvert(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateGdptAdvert(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGdptAdvert(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('shop/gdptAdvert/export', {
        ...this.queryParams
      }, `gdptAdvert_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
