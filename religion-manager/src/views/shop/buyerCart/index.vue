    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['shop:buyerCart:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['shop:buyerCart:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['shop:buyerCart:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="buyerCartList" @selection-change="handleSelectionChange">
...
<script>
import { listBuyerCart, getBuyerCart, delBuyerCart, addBuyerCart, updateBuyerCart } from "@/api/shop/buyerCart";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listBuyerCart(this.queryParams).then(response => {
        this.buyerCartList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getBuyerCart(cartId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.cartId != null) {
            updateBuyerCart(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addBuyerCart(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('shop/buyerCart/export', {
        ...this.queryParams
      }, `buyerCart_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
